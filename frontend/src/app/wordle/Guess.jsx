...
function Tile({ letter, colour, flip, index, shake }) {
  ...
  return (
    ...
    >
      <div
        className="tile front bg-black text-white"
        style={{ animationDelay: delayFlip }}
      >
        {letter}
      </div>

      <div
        className={`tile back ${colour} text-white`}
        style={{ animationDelay: delayFlip }}
      >
        {letter}
      </div>
  ...
  return (
    <div className="grid grid-cols-5 gap-2 mb-2">
      {new Array(5).fill("").map((_, i) => {
        ...
        return version === "blindle" && customRow[i] ? (
          <Tile
            key={i}
            letter={"X"}
            colour={"bg-black"}
